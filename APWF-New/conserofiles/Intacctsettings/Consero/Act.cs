...
namespace Consero.Plugins.Custom
{
        ...
        IPluginProjectDisplay, IPluginProjectCommit, IPluginDatabase, IPluginRawPageDisplay
    {
        ...
        {
            api.Notifications.AddMessage(Project.ToString(), "1a");
            ...
            api.Notifications.AddMessage(Project.ToString(), "1b");
            if ("1".Equals(preCommitEnableIntacct))
            {
             ...
             //   if (string.IsNullOrEmpty(preCommitIntacctLocationId))
              ...
              //  }
                ...
                {
                    api.Notifications.AddMessage(Project.ToString(),"1");
       ...
       //    api.Notifications.AddMessage("xml", project +sType+ sSenderId+ sSenderPassword+ sCompanyId+ sUserId+ sUserPassword+ sLocationId);
          
            MemoryStream ms = Xml(project, sType, sSenderId, sSenderPassword, sCompanyId, sUserId, sUserPassword, sLocationId);

            if (ms == null)
            {
                api.Notifications.AddMessage("9pf");
                api.Notifications.AddAdminNotification("ms has null value", "1");
            }
            else
            {
                api.Notifications.AddMessage("10pf");
                api.Notifications.AddAdminNotification("ms has value", "2");
            }
          ...
          //  objHttpWebRequest = (HttpWebRequest)WebRequest.Create(sIntacctUrl);
            ...
            try
            {
                ...
                api.Notifications.AddMessage("msl",ms.Length.ToString());
              ...
              //  api.Notifications.AddMessage("2f");
                ...
                {
                    //Get Stream object 
                    api.Notifications.AddMessage("1f", objHttpWebRequest.Address.ToString());

                    objRequestStream = objHttpWebRequest.GetRequestStream();
                     
                    if (objHttpWebRequest.HaveResponse)
                    {
                      
                            
                        api.Notifications.AddMessage("9tf");
                        api.Notifications.AddAdminNotification("objHttpWebRequest has null value", "1");
                    }
                    else
                    {
                        api.Notifications.AddMessage("10tf");
                        api.Notifications.AddAdminNotification("objHttpWebRequest has value", "2");
                    }
                ...
                api.Notifications.AddMessage("3f");
                objHttpWebResponse = (HttpWebResponse)objHttpWebRequest.GetResponse();
                api.Notifications.AddMessage("4f");
                //---------- Start HttpResponse
              
                if (objHttpWebResponse.StatusCode == HttpStatusCode.OK)
                {
                    api.Notifications.AddMessage("5f");
                    //Get response stream 
                    objResponseStream = objHttpWebResponse.GetResponseStream();

                    if (null == objResponseStream)
                    {
                        api.Notifications.AddMessage("9af");
                        api.Notifications.AddAdminNotification("objResponseStream has null value", "1");
                    }
                    else
                    {
                        api.Notifications.AddMessage("10af");
                        api.Notifications.AddAdminNotification("objResponseStream has value", "2");
                    }

                    //Load response stream into XMLReader
                    objXMLReader = new XmlTextReader(objResponseStream);
                    api.Notifications.AddMessage("6f");
                    if ( objXMLReader.HasValue)
                    {
                        api.Notifications.AddMessage("9f");
                        api.Notifications.AddAdminNotification("objXMLReader has null value", "1");
                    }
                    else
                    {
                        api.Notifications.AddMessage("10f");
                        api.Notifications.AddAdminNotification("objXMLReader has value", "2");
                    }
                    //Declare XMLDocument
                    XmlDocument xmldoc = new XmlDocument();


                    api.Notifications.AddMessage("7fb", objXMLReader.Value);
                    xmldoc.Load(objXMLReader);
                    api.Notifications.AddMessage("7f");
                    //Set XMLResponse object returned from XMLReader
                    xmlResponse = xmldoc;
                    api.Notifications.AddMessage("8f");
                  
                    api.Notifications.AddMessage("11f");
                    //Close XMLReader
                    objXMLReader.Close();
                }
            ...
            api.Notifications.AddMessage("vendor ID is." + sType);
            //Return
            return xmlResponse;
        }
        ...
        {
            api.Notifications.AddAdminNotification("xmlResponse has null value", "3");
        ...
        {
            api.Notifications.AddMessage("Project ID is." + ixProject);
        ...
        //   api.Notifications.AddMessage("Intacct Client Id is Required.");
           ...
           // xw.WriteString(sUserId);
            ...
            xw.WriteString("Silverback");
            xw.WriteEndElement();
            xw.WriteStartElement("password");
           // xw.WriteString(sUserPwd);
            xw.WriteString("Welcome123$$");
            xw.WriteEndElement();
