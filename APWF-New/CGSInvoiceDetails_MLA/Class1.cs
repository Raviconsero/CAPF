        ...
        IPluginBugDisplay, IPluginBugCommit, IPluginDatabase, IPluginRawPageDisplay, IPluginGridColumn, IPluginJS
    {
            ...
            bool fPublic)
        {
            ...
            if (flag == 1)
            {
              ...
              //  api.Notifications.AddAdminNotification("CCuser", flag.ToString());
                if ((bug.GetPluginField("customfields@fogcreek.com", "emailxccx013")) != null)
                {
                  //  api.Notifications.AddAdminNotification("CCuser1", flag.ToString());
                    if (bug.GetPluginField("customfields@fogcreek.com", "emailxccx013").ToString() != "")
                    {
                        mailsub1 = "CC- An Invoice is sent for Approval";
                        mailbody1 = "There is an invoice Sent for Approval.  Please log in here to see the details: http://empower.conseroglobal.com/default.asp?" + bug.ixBug; ;
                       // api.Notifications.AddAdminNotification("CCuser2", flag.ToString());
                         CCemail = (bug.GetPluginField("customfields@fogcreek.com", "emailxccx013")).ToString();
                       // api.Notifications.AddAdminNotification("CCuser3", CCemail.ToString());
                        mailsender(CCemail.Trim(), bug, mailsub1, mailbody1, ixperson);
                       // api.Notifications.AddAdminNotification("CCuser4", CCemail.ToString());
                        this.api.Notifications.AddMessage("A CCEmail has been sent Successfully");
                       // api.Notifications.AddAdminNotification("CCuser5", CCemail.ToString());
                            ...
                            if (sL1 != "-")
                            {
                                ...
                                if (L1 == L0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        string CCemail = (bug.GetPluginField("customfields@fogcreek.com", "emailxccx013")).ToString();
                                        if (bug.ixPersonResolvedBy == 341)
                                        {

                                            if (bug.ixStatus == 70)
                                            {
                                               // api.Notifications.AddAdminNotification("Approved", bug.ixStatus.ToString());
                                                 Proc_mailsub = "An Invoice has been Approved";
                                                 Proc_mailbody = "CC - There is an invoice which has been Approved.  Please log in here to see the details: http://empower.conseroglobal.com/default.asp?" + bug.ixBug; ;
                                                mailsender(CCemail.Trim(), bug, Proc_mailsub, Proc_mailbody, ixperson);
                                                //api.Notifications.AddAdminNotification("Approved1", bug.ixStatus.ToString());
                                                this.api.Notifications.AddMessage("A CCEmail has been sent Successfully");
                                            }
                                       }
                        ...
                        if (bug.ixStatus == 73)
                        {
                            ...
                            if (L0 == L1)
                            {
                                ...
                                if (bug.ixPersonResolvedBy == 341)
                                {
                                    ...
                                    if (bug.ixStatus == 73)
                                    {
                                       // api.Notifications.AddAdminNotification("Rejected", bug.ixStatus.ToString());
                                         Rej_mailsub = "An Invoice has been Rejected";
                                         Rej_mailbody = "CC - There is an invoice which has been.  Please log in here to see the details: http://empower.conseroglobal.com/default.asp?" + bug.ixBug; ;
                                        mailsender(CCemail.Trim(), bug, Rej_mailsub, Rej_mailbody, ixperson);
                                      //  api.Notifications.AddAdminNotification("Rejected2", bug.ixStatus.ToString());
                                        this.api.Notifications.AddMessage("A CCEmail has been sent Successfully");
                                    }
                                   ...
                                   // api.Notifications.AddAdminNotification("status2", bug.ixStatus.ToString());
                                    if (bug.ixPersonResolvedBy != 299)
                                    {
                                      //  api.Notifications.AddAdminNotification("ixPersonResolvedBy", bug.ixPersonResolvedBy.ToString());
                                        ...
                                        string Finance_mailbody = "A PO has been Approved for vendor : " + vendor_1 + ", amount: $" + amt.ToString() + ", description :" + title + ". Click on the link to review the PO information and generate Purchase Order http://empower.conseroglobal.com/default.asp?" + bug.ixBug;
                                        //  string Proc_mailbody = "The PO you submitted has been approved and new PO number " + PONumb + " has been created, accounting team will share the purchase order copy shortly.";
                                        mailsender_Finance("vittal.r@conseroglobal.com", bug, Finance_mailsub, Finance_mailbody, iperson);
                                        bug.ixStatus = 183;
        ...
        protected void InsertItem_POInvoice()
        {
            try
            {
             ...
             //   api.Notifications.AddAdminNotification("sInvoiceDate", api.Request[api.AddPluginPrefix("ixProject")].ToString());
                ...
                CSelectQuery POTotAmt = api.Database.NewSelectQuery(api.Database.PluginTableName("CGSPOMatchedInvoice"));
                POTotAmt.AddSelect("SUM(fAmount) as InvSumAcc");
                POTotAmt.AddWhere("ixBug = " + api.Request[api.AddPluginPrefix("ixBug")].ToString());
                POTotAmt.AddWhere("ixLineItemId_Inv = " + Convert.ToInt32(api.Request[api.AddPluginPrefix("ixLineItemId_Inv")]));

              //  api.Notifications.AddAdminNotification("CGSPOMatchedInvoice", "1");
                DataSet TotalInvLineAmt = POTotAmt.GetDataSet();
              ...
              //  api.Notifications.AddAdminNotification("CGSPOMatchedInvoice5", "5");
                ...
                double POTotalAmt = 0d;
                double POBalance = 0d;
                double Balanceamt = 0d;

                CSelectQuery TotaPoamt = api.Database.NewSelectQuery(api.Database.PluginTableName("CGSInvoice_MLA"));
                TotaPoamt.AddSelect("Add_Fld2");
                string sWhere2 = (api.Database.PluginTableName("CGSInvoice_MLA")) + ".ixBug = " + api.Request[api.AddPluginPrefix("ixBug")].ToString();
                TotaPoamt.AddWhere(sWhere2);
                object POAmt = TotaPoamt.GetScalarValue();
                POTotalAmt = Convert.ToDouble(POAmt);

                CSelectQuery TotalInv = api.Database.NewSelectQuery(api.Database.PluginTableName("CGSPOMatchedInvoice"));
                TotalInv.AddSelect("SUM(fAmount) as InvSumAcc");
                TotalInv.AddWhere("ixBug = " + api.Request[api.AddPluginPrefix("ixBug")].ToString());
                object InvAmt = TotalInv.GetScalarValue();
                Balanceamt = Convert.ToDouble(InvAmt);

                api.Notifications.AddAdminNotification("POTotalAmt", POTotalAmt.ToString());
                POBalance = (POTotalAmt) - (Balanceamt);


                int id = 0;
                CSelectQuery LId = api.Database.NewSelectQuery(api.Database.PluginTableName("CGSPOMatchedInvoice"));
                LId.AddSelect("MAX(ixBugLineItem_PMI)");
                // LId.AddWhere("ixBug = " + api.Request[api.AddPluginPrefix("ixBug")].ToString());
                object NId = LId.GetScalarValue();
                id = Convert.ToInt32(NId);

                api.Notifications.AddAdminNotification("id", id.ToString());
                
                //insert.InsertFloat("sExtra2", SubtotalAmt);
                api.Notifications.AddAdminNotification("POBalance", POBalance.ToString());
                CUpdateQuery update1 = api.Database.NewUpdateQuery(api.Database.PluginTableName("CGSInvoiceDetails_MLA@conseroglobal.com", "CGSPOMatchedInvoice"));
                update1.UpdateFloat("sExtra2", POBalance);
                update1.AddWhere("ixLineItemId_Inv = " + Convert.ToInt32(api.Request[api.AddPluginPrefix("ixLineItemId_Inv")]));
                update1.AddWhere("ixBugLineItem_PMI = " + id.ToString());
                api.Notifications.AddAdminNotification("ixLineItemId_Inv1", (api.Request[api.AddPluginPrefix("ixLineItemId_Inv")]).ToString());
                update1.AddWhere("ixBug =" + api.Request[api.AddPluginPrefix("ixBug")].ToString());
                update1.Execute();
